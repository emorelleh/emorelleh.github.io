---
layout: default
title: Research
description: "Research on green and blue criminology, environmental criminal law, marine harm, and ecological justice"
permalink: /research/
---

<div class="research-page">

  <article class="research-content">

    <header class="research-intro">
      <h1>Research</h1>
    </header>

    <section id="research-perspective">
      <h2>Research perspective</h2>

      <p>
        I study how criminal law and criminology respond to damage affecting
        ecosystems, marine life, and the communities connected to them.
        My work brings legal analysis into conversation with ecological evidence.
      </p>

      <p>
        Environmental harm does not always fit neatly into legal definitions of crime.
        Damage to an ecosystem can accumulate over time, involve several activities,
        and become visible only after ecological conditions have changed. A response
        focused solely on individual offences may miss these wider processes.
      </p>

      <p>
        My research asks what criminal law can do in these circumstances, where its
        limits lie, and how criminology can help identify harms that remain outside
        formal definitions of illegality. I approach these questions through green
        and blue criminology, environmental criminal law, ecological law, and marine
        ecology.
      </p>

      <p>
        I am particularly interested in the relationship between legal categories
        and ecological realities: how harm is measured, whose interests are protected,
        what counts as evidence, and which forms of accountability or restoration
        are possible.
      </p>
    </section>

    <section id="research-interests">
      <h2>Research interests</h2>

      <div class="research-area">
        <h3>Green and blue criminology</h3>
        <p>
          I examine environmental crime and harm on land and at sea, including
          marine pollution, illegal fishing, biodiversity loss, and the effects
          of human activity on coastal and ocean ecosystems.
        </p>
      </div>

      <div class="research-area">
        <h3>Environmental criminal law and ecocide</h3>
        <p>
          This work considers how criminal law defines ecological damage and
          protects ecosystems. It includes research on environmental offences,
          ecocentric interpretations of legal interests, and proposals concerning
          ecocide as an international crime.
        </p>
      </div>

      <div class="research-area">
        <h3>Marine ecosystems and environmental evidence</h3>
        <p>
          My marine-focused research addresses underwater noise, brine discharges,
          impacts on <em>Posidonia oceanica</em>, and methods for identifying
          ecological damage. I am interested in how biological and ecological
          evidence can inform legal and criminological analysis.
        </p>
      </div>

      <div class="research-area">
        <h3>Climate change and planetary boundaries</h3>
        <p>
          I explore how climate change and planetary boundaries challenge
          established approaches to criminal policy, environmental protection,
          security, and responsibility for socioecological harm.
        </p>
      </div>

      <div class="research-area">
        <h3>Ecological and restorative justice</h3>
        <p>
          This line of work considers ecosystems as affected entities and asks
          what meaningful repair, prevention, and accountability might look
          like after environmental harm.
        </p>
      </div>
    </section>

    <section id="selected-projects">
      <h2>Selected projects</h2>

      <div class="research-project">
        <h3>
          <a href="https://www.uji.es/serveis/ocit/base/grupsinvestigacio/detall/?codi=165&amp;p_idioma=en&amp;antiguedad=10">
            CRIMICLIMA
          </a>
        </h3>
        <p class="project-meta">
          Principal Investigator · Universitat Jaume I · 2024–2026
        </p>
        <p>
          A criminological analysis of the effectiveness of environmental
          criminal policy in the context of the climate emergency.
          The UJI research-centre record includes the project under code
          UJI-2023-02.
        </p>
      </div>

      <div class="research-project">
        <h3>
          <a href="https://ocn.uib.eu/team/">
            Ocean Crime Narratives
          </a>
        </h3>
        <p class="project-meta">
          Research team · University of the Balearic Islands · 2022–2027
        </p>
        <p>
          A study of the narratives through which environmental crime and
          harm at sea have been understood and represented. My profile is
          listed on the project's team page.
        </p>
      </div>

      <div class="research-project">
        <h3>Blue Governance Laboratory (BlueGovLab)</h3>
        <p class="project-meta">
          Scientific Coordinator · Universitat Jaume I · Since 2025
        </p>
        <p>
          An interdisciplinary laboratory connecting research on ocean
          governance, environmental protection, marine biodiversity,
          and public policy.
        </p>
      </div>

      <div class="research-project">
        <h3>
          <a href="https://ibizapreservation.org/la-comision-europea-reconoce-el-aula-ibizapreservation-de-criminalidad-azul-de-la-uji-como-accion-oficial-de-la-mision-restore-our-ocean-and-waters/">
            Aula IbizaPreservation de Criminalidad Azul
          </a>
        </h3>
        <p class="project-meta">
          Director · Universitat Jaume I · Since 2025
        </p>
        <p>
          A collaboration focused on marine environmental harm in Ibiza
          and Formentera through research, training, and knowledge exchange.
        </p>
      </div>
    </section>

    <section id="methods-and-collaboration">
      <h2>Methods and collaboration</h2>

      <p>
        My work combines legal and case-law analysis with criminological
        approaches to environmental harm. It also engages with ecological
        indicators, marine research, and forms of evidence that can help
        describe damage beyond the boundaries of a single legal case.
      </p>

      <p>
        These questions require collaboration across disciplines. I work
        with researchers in law, criminology, marine ecology, environmental
        science, and related fields, as well as with organisations concerned
        with the protection of marine and coastal environments.
      </p>
    </section>

    <section id="research-outputs">
      <h2>Research outputs</h2>

      <p>
        My <a href="{{ '/publications/' | relative_url }}">publications</a>
        address underwater noise, marine pollution, environmental criminal
        law, ecocide, <em>Posidonia oceanica</em>, and green criminology.
        A broader record is available through my
        <a href="https://portalcientific.uji.es/investigadores/1327563/publicaciones">
          UJI research profile
        </a>.
      </p>

      <p>
        Public research materials include a
        <a href="https://zenodo.org/records/15024172">
          dataset on Spanish environmental-crime judgments concerning
          Article 325 of the Criminal Code (2015–2024)
        </a>. Further datasets are listed on the
        <a href="https://portalcientific.uji.es/datos?personaId=1327563">
          UJI research-data portal
        </a>.
      </p>
    </section>

  </article>

  <aside class="research-toc" aria-label="On this page">
    <p>On this page</p>
    <a href="#research-perspective">Research perspective</a>
    <a href="#research-interests">Research interests</a>
    <a href="#selected-projects">Selected projects</a>
    <a href="#methods-and-collaboration">Methods and collaboration</a>
    <a href="#research-outputs">Research outputs</a>
  </aside>

</div>
